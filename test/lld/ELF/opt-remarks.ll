...
; RUN:   %t.o -o %t -shared
; RUN: %link %linkopts --opt-remarks-with-hotness --opt-remarks-hotness-threshold=300 \
; RUN:   --opt-remarks-filename %t.t300.yaml %t.o -o %t -shared
; RUN: %link %linkopts --opt-remarks-with-hotness --opt-remarks-hotness-threshold=301 \
...
; RUN: %link %linkopts --opt-remarks-filename %t1.yaml --opt-remarks-passes inline %t.o \
; RUN:   -o %t -shared
; RUN: cat %t1.yaml | FileCheck %s -check-prefix=YAML-PASSES
; RUN: %link %linkopts --opt-remarks-filename %t1.yaml --opt-remarks-format yaml %t.o \
; RUN:   -o %t -shared
; RUN: FileCheck %s -check-prefix=YAML < %t1.yaml
; RUN: %link %linkopts --opt-remarks-filename %t.bit --opt-remarks-format bitstream %t.o \
; RUN:   -o %t -shared
; RUN: llvm-remarkutil bitstream2yaml %t.bit | FileCheck %s -check-prefix=YAML
